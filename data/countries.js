...
//
// 45 entries: 44 UN-recognized landlocked states plus Kosovo (XKX, partially
// recognized but treated as a sovereign country throughout this app — same
// as how it appears in COUNTRIES). v5.7 audit added AFG, which had been
// missing — Afghanistan is unambiguously landlocked (the previous "44"
// count was correct in total but had AFG/XKX swapped).
//
// Re-landlocked since independence: ETH (lost coast when Eritrea seceded
// in 1993), SSD (born landlocked when South Sudan separated in 2011).
// AZE is "landlocked" in the no-ocean-access sense — its Caspian coastline
// is on a closed sea, so it counts as landlocked by every standard list.
export const LANDLOCKED = new Set([
  'AFG','AND','ARM','AUT','AZE','BLR','BTN','BOL','BWA','BFA','BDI','CAF','TCD','CZE',
  'SWZ','ETH','HUN','KAZ','XKX','KGZ','LAO','LSO','LIE','LUX','MWI','MLI','MDA','MNG',
  'NPL','NER','MKD','PRY','RWA','SMR','SRB','SVK','SSD','CHE','TJK','TKM','UGA','UZB',
  'VAT','ZMB','ZWE',
]);
...
// Powers the Islander line. 48 entries from Iceland in the Atlantic to the
// Pacific microstates.
//
// Inclusion calls (countries that share at least one land border, kept
// because their identity is overwhelmingly island-based):
//   • HTI/DOM share Hispaniola — both kept
//   • IDN shares Borneo with MYS/BRN, New Guinea with PNG, Timor with TLS,
//     but its national identity is the archipelago — kept
//   • BRN sits on Borneo with MYS/IDN — kept (small enclave on one island)
//   • PNG shares New Guinea with IDN — kept
//   • TLS shares Timor with IDN — kept (half an island)
//   • AUS is a continent, but is also a single landmass with no neighbour —
//     kept (the Islander line pays out on AUS visits, deliberately)
// MYS is excluded — half its territory is on the Malay Peninsula (mainland
// Asia, sharing land with THA), so it's not pure island.
...
// v5.7 audit: SOM removed from `equator` — Somalia's southernmost point is
// ~1.6°N (Ras Kamboni at the Kenya border), so the equator does not cross
// Somali territory. KEN is the southernmost equator-crosser on Africa's
// east coast.
export const GEO_SETS = {
  equator:  ['BRA','COL','ECU','STP','GAB','COG','COD','UGA','KEN','IDN'],
